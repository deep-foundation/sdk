...
export default async function checkCameraPermission(deep: DeepClient, deviceLinkId) {
    ...
    const cameraPermissionTypelinkId = await deep.id(CAPACITOR_CAMERA_PACKAGE_NAME, "CameraPermissions");
    const photosPermissionTypelinkId = await deep.id(CAPACITOR_CAMERA_PACKAGE_NAME, "PhotoPermissions");
